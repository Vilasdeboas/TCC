...
public class Unit : MonoBehaviour {
    ...
    public void SetInfo() {
        ...
        if(isPlayer) {
            ...
            experience = new System.Random().Next(unitLevel, unitLevel + 2);
            //gameObject.GetComponentInChildren<SpriteRenderer>().sprite = GetSpriteByUnitName(unitName);
            gameObject.GetComponentInChildren<SpriteRenderer>().sprite = SpriteManager.GetUnitSpriteList(FormatUnitName(unitName))[0];
    ...
    private Sprite GetSpriteByUnitName(string name) {
        name = FormatUnitName(name);
        Sprite sprite = Resources.Load<Sprite>(unitsPath+ name + "/"+ name+ "_idle1");
        return sprite;
    }

    private string FormatUnitName(string name) {
